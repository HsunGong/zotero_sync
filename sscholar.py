...
def retrieve_info(article, zot):
    ...
    if len(results) == 0:
        return template
    best_res = results[0]
    best_res = defaultdict(lambda:defaultdict(str), results[0])
    #best_res["publicationVenue"] = best_res.get("publicationVenue", {})
    #best_res["journal"] = best_res.get("journal", {})
    #best_res["authors"] = best_res.get("authors", {})
